...
<div class="max-w-2xl mx-auto ui-animate-entry">
    <form method="post" class="ui-panel rounded-2xl overflow-hidden shadow-lg border border-slate-200 dark:border-stone-700/60 bg-white dark:bg-[#0f172a]">
        <?= csrf_field() ?>
        <input type="hidden" name="id" value="<?= (int) $A['id'] ?>">

        <!-- Header: Student Info -->
        <div class="px-6 py-6 border-b border-slate-200 dark:border-stone-800 relative overflow-hidden">
            <!-- Decorative background -->
            <div class="absolute inset-0 opacity-20 pointer-events-none" style="background: linear-gradient(135deg, #26263A, transparent);"></div>
            
            <div class="flex items-center gap-4 relative z-10">
                <div class="w-14 h-14 rounded-full flex items-center justify-center text-xl font-bold text-white shrink-0 shadow-md border-2 border-white/10" style="background: linear-gradient(135deg, #26263A, #7A7AA3);">
                    <?= mb_strtoupper(mb_substr($A['nombres'], 0, 1)) ?>
                </div>
                <div>
                    <h2 class="text-xl font-display font-bold text-slate-800 dark:text-stone-100 leading-tight">
                        <?= e(nombre_completo($A['nombres'], $A['apellidos'])) ?>
                    </h2>
                    <div class="flex flex-wrap items-center gap-3 mt-1 text-sm">
                        <span class="flex items-center gap-1 text-slate-500 dark:text-stone-400 font-mono">
                        ...
                        <span class="flex items-center gap-1 text-[#26263A] font-semibold bg-[#26263A]/10 px-2.5 py-0.5 rounded-md">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="grid sm:grid-cols-2 gap-5">
                <!-- Entrada -->
                <div class="bg-slate-50 dark:bg-stone-800/40 p-4 rounded-xl border border-slate-100 dark:border-stone-700/50">
                    <div class="flex justify-between items-center mb-2">
                        <label class="text-sm font-semibold text-slate-700 dark:text-stone-300 flex items-center gap-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="time" step="1" name="hora_entrada" id="inEntrada" value="<?= e(substr((string) ($A['hora_entrada'] ?? ''), 0, 8)) ?>" 
                               class="w-full pl-9 pr-3 py-2.5 rounded-lg border border-slate-300 dark:border-stone-600 bg-white dark:bg-stone-900 text-slate-800 dark:text-stone-200 font-mono focus:outline-none focus:ring-2 focus:ring-[#26263A]/30 focus:border-[#26263A] transition-all">
                    </div>
                </div>
            ...
            <div class="grid sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-stone-300 mb-1.5">Estado de la asistencia</label>
                    <select name="estado" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-stone-600 bg-white dark:bg-stone-900 text-slate-800 dark:text-stone-200 focus:outline-none focus:ring-2 focus:ring-[#26263A]/30 focus:border-[#26263A] transition-all appearance-none cursor-pointer">
            ...
            <div class="pt-4 flex flex-col sm:flex-row gap-3">
                <button type="submit" name="accion" value="guardar" class="flex-1 sm:flex-none inline-flex justify-center items-center gap-2 rounded-xl py-2.5 px-6 font-semibold text-white transition-all hover:brightness-110 active:scale-[0.98]" style="background: linear-gradient(135deg, #26263A, #7A7AA3);">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                    Guardar Cambios
                </button>
                <a href="<?= e(app_url('index.php?r=asistencia')) ?>" class="flex-1 sm:flex-none inline-flex justify-center items-center gap-2 rounded-xl border border-slate-300 dark:border-stone-600 bg-transparent py-2.5 px-6 font-semibold text-slate-700 dark:text-stone-300 transition-all hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-[0.98]">
                    Cancelar
                </a>
            </div>
        </div>
